...
pub enum SchemaError {
    ...
    #[error("unsupported bundled schema format version {actual}; this build expects {expected}")]
    BundledFormatVersion { expected: u32, actual: u32 },
    #[error("bundled schema has no source revision")]
    MissingBundledSourceRevision,
    #[error(
        "bundled schema was generated by a different extractor revision \
         (artifact {artifact:#018x}, current {current:#018x})"
    )]
    BundledGeneratorMismatch { artifact: u64, current: u64 },
}
...
pub const BUNDLED_SCHEMA_FORMAT_VERSION: u32 = 1;
const BUNDLED_SCHEMA_BYTES: &[u8] = include_bytes!("../generated/object-registry.json");
...
/// Versioned, source-free metadata shipped with Graffito.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct BundledObjectRegistry {
    pub format_version: u32,
    pub source_revision: String,
    pub source_fingerprint: u64,
    pub generator_fingerprint: u64,
    pub registry: ObjectRegistry,
}

impl BundledObjectRegistry {
    ...
    pub fn to_pretty_vec(&self) -> Result<Vec<u8>> {
        self.validate()?;
    ...
    pub fn validate(&self) -> Result<()> {
        if self.format_version != BUNDLED_SCHEMA_FORMAT_VERSION {
            return Err(SchemaError::BundledFormatVersion {
                expected: BUNDLED_SCHEMA_FORMAT_VERSION,
                actual: self.format_version,
            });
        }
        if self.source_revision.trim().is_empty() {
            return Err(SchemaError::MissingBundledSourceRevision);
        }
        let current = schema_generator_fingerprint();
        if self.generator_fingerprint != current {
            return Err(SchemaError::BundledGeneratorMismatch {
                artifact: self.generator_fingerprint,
                current,
            });
        }
...
/// Loads and validates the source-free registry embedded in this build.
pub fn bundled_object_registry() -> Result<BundledObjectRegistry> {
    BundledObjectRegistry::from_slice(BUNDLED_SCHEMA_BYTES)
}
...
impl SchemaGenerator {
    ...
    /// Generates the versioned artifact embedded in distributable builds.
    pub fn generate_bundle(
        &self,
        source_revision: impl Into<String>,
    ) -> Result<BundledObjectRegistry> {
        let source_revision = source_revision.into();
        if source_revision.trim().is_empty() {
            return Err(SchemaError::MissingBundledSourceRevision);
        }
        let sources = SourceInventory::build(&self.repo_root)?;
        let source_fingerprint = sources.content_fingerprint();
        let registry = self.generate_from_inventory(&sources)?;
        Ok(BundledObjectRegistry {
            format_version: BUNDLED_SCHEMA_FORMAT_VERSION,
            source_revision,
            source_fingerprint,
            generator_fingerprint: schema_generator_fingerprint(),
            registry,
        })
    }
...
mod tests {
    ...
    #[test]
    fn shipped_schema_bundle_is_current_complete_and_deterministic() {
        let bundle = bundled_object_registry().unwrap();

        assert_ne!(bundle.source_revision, "placeholder");
        assert!(!bundle.registry.objects.is_empty());
        ...
        assert_eq!(serialized.as_slice(), BUNDLED_SCHEMA_BYTES);
    }

    #[test]
    fn bundle_rejects_mismatched_generator_fingerprint() {
        let mut bundle = bundled_object_registry().unwrap();
        bundle.generator_fingerprint ^= 1;

        assert!(matches!(
            bundle.validate(),
            Err(SchemaError::BundledGeneratorMismatch { .. })
        ));
    }
